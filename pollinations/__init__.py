...
class Text(object):
    ...
    def __call__(self, 
        ...
        self.messages = [Text.Message(message.get("role", "user"), message.get("content", "")) if isinstance(message, dict) else message for message in self.messages]
        ...
        if display is True:
            for i, char in enumerate(self.response):
                delay = (0.1, 0.3) if i > 0 and self.response[i - 1] not in set(
        ...
        def __call__(self, encode: bool = False, *args, **kwargs):
            try:
                if self.contextual:
                    ...
                    if self.system and (not messages or messages[0]["role"] != "system"):
                        system_message = Text.Message("system", self.system)()
                        messages.insert(0, system_message)
                    
                    if self.prompt is not None:
                        if self.images is not None and len(self.images) > 0:
                            messages.append(Text.Message("user", self.prompt, self.images)())
                        else:
